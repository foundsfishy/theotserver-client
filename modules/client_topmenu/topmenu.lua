...
-- exposed so client_entergame can drive the dev host toggle that lives on the top bar
function getDevHostButton()
  return devHostBtn
end
...
function init()
  ...
  -- Dev-only host switch pill, placed on the top bar next to Discord. Only created
  -- when host_local.lua set DEV_MODE, so players never see it. EnterGame fills in the
  -- text/colour and handles the actual switch.
  if DEV_MODE and topMenu.discordLabel then
    devHostBtn = g_ui.createWidget('DevHostToggle', topMenu)
    devHostBtn:addAnchor(AnchorVerticalCenter, 'parent', AnchorVerticalCenter)
    devHostBtn:addAnchor(AnchorLeft, 'discordLabel', AnchorRight)
    devHostBtn:setMarginLeft(10)
    devHostBtn:setText('LIVE')
    devHostBtn.onClick = function()
      if EnterGame and EnterGame.toggleDevHost then EnterGame.toggleDevHost() end
    end
  end
