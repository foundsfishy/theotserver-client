...
-- Reorder every tab (including the overflow tabs hidden in preTabs/postTabs)
-- according to compareFunction, then relayout. compareFunction(tabA, tabB) must
-- return true when tabA should appear to the left of tabB. The current selection
-- is preserved.
function UIMoveableTabBar:sortTabs(compareFunction)
  -- pull any hidden overflow tabs back into the visible list so the full set is
  -- sorted as one sequence (leftmost preTab first, rightmost postTab last)
  while #self.preTabs > 0 do
    local tab = table.remove(self.preTabs)
    tab:setVisible(true)
    table.insert(self.tabs, 1, tab)
  end
  while #self.postTabs > 0 do
    local tab = table.remove(self.postTabs, 1)
    tab:setVisible(true)
    table.insert(self.tabs, tab)
  end

  table.sort(self.tabs, compareFunction)

  -- relayout exactly like a geometry change would: re-hide whatever no longer fits
  updateMargins(self)
