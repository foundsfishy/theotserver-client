...
MessageTypes = {
  ...
  -- Loot text goes ONLY to the dedicated Loot tab (mods/game_lootchannel), which
  -- listens for it independently of this table. `none` keeps it out of Server
  -- Log and off the center-screen popup so it isn't shown twice.
  [MessageModes.Loot] = MessageSettings.none,
...
function displayMessage(mode, text)
  ...
  -- Loot lines ("Loot of <monster>: ...") belong ONLY in the dedicated Loot tab
  -- (mods/game_lootchannel listens for them independently). Suppress them here so
  -- they don't ALSO echo to the Server Log console and the center-screen popup.
  -- Filtering by text (not a wire mode) is deliberate: the server sends loot as a
  -- normal valid info message - a custom "loot" wire mode has no valid 8.60 code
  -- and crashes the client packet parser.
  if text and string.find(text, '^Loot of ') then return end
