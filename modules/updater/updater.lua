...
local function updateFiles(data, keepCurrentFiles)
  ...
  downloadFiles(data["url"], toUpdate, 1, 0, function()
    ...
    scheduledEvent = scheduleEvent(function()
      -- Always restart when ANY file was updated (newFiles), not just core files,
      -- so data-only changes (backgrounds, images, styles, spells, etc.) take effect
      -- immediately instead of on the next manual restart.
      local restart = newFiles or binary or (not loadModulesFunction and reloadModules) or forceRestart
      if newFiles then
