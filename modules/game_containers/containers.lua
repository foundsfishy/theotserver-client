...
-- Server hard cap on simultaneously open containers (cid 0..15 rejected in
-- Player::addContainer). A 17th open request is silently ignored server-side,
-- so we recycle windows client-side instead (see openSmart below).
local MAX_OPEN_CONTAINERS = 16
-- Remembers the last openSmart request so onContainerOpen can tag the new
-- container with where it was opened from (map/ground vs inventory).
...
-- Open a container into a NEW window (nothing to navigate in-place from).
-- At the server's 16-container cap a fresh open would be silently ignored, so
-- instead we recycle the window of the OLDEST container that was opened from
-- the map (corpse / loot bag) - like the real Tibia client. Inventory
-- backpacks are never touched. Used by game_interface for all map/inventory
-- container opens.
function openSmart(thing)
  ...
  local recycleInto = nil
  if count >= MAX_OPEN_CONTAINERS then
    for _, container in pairs(containers) do
      if container.openedFromMap and
         (not recycleInto or (container.openedAt or 0) < (recycleInto.openedAt or 0)) then
        recycleInto = container
      end
    end
    if not recycleInto then
      -- every open window is an inventory container; don't fail silently
      modules.game_textmessage.displayFailureMessage(tr('You cannot open more containers. Close one first.'))
      return
    end
  end
  ...
  pendingOpen = {
    itemId = thing:getId(),
    fromMap = pos ~= nil and pos.x ~= 65535, -- 65535 = inventory/container slot
    at = g_clock.millis()
  }
  g_game.open(thing, recycleInto)
end
...
function onContainerOpen(container, previousContainer)
  ...
  -- tag for openSmart's window recycling: when and from where it was opened.
  -- In-place navigation (child bag / parent) inherits the original source, so
  -- a bag opened inside a corpse still counts as map loot.
  container.openedAt = g_clock.millis()
  if previousContainer then
    container.openedFromMap = previousContainer.openedFromMap
  end
  if pendingOpen and g_clock.millis() - pendingOpen.at < 2000
     and container:getContainerItem():getId() == pendingOpen.itemId then
    container.openedFromMap = pendingOpen.fromMap
    pendingOpen = nil
  end
