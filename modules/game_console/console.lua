...
-- === Console tab ordering (persisted across launches) ========================
-- The three lead tabs are always anchored left-to-right: Default, Server Log,
-- Loot. Every other channel (Tips, World Chat, NPCs, ...) keeps the exact order
-- the player last arranged them in, remembered across launches. On every login
-- the player always lands on the Default tab.
local tabReorderEvent = nil
local LOOT_TAB_NAMES = { ['Loot'] = true, ['vBot Loot'] = true }

-- Fixed leading slot for an anchor tab, or nil for an ordinary channel.
local function anchorRank(name)
  if name == tr('Default') then return 1 end
  if name == tr('Server Log') then return 2 end
  if LOOT_TAB_NAMES[name] then return 3 end
  return nil
end

-- A channel whose position the player can arrange and we persist: any real chat
-- channel, or the NPCs tab. Anchors and private-message tabs are excluded.
local function isPersistableTab(tab)
  if not tab then return false end
  if anchorRank(tab:getText()) then return false end
  if tab.channelId ~= nil then return true end
  return tab:getText() == 'NPCs'
end

function saveTabOrder()
  if not consoleTabBar then return end
  local order = {}
  local function collect(list)
    if not list then return end
    for _, tab in ipairs(list) do
      if isPersistableTab(tab) then
        table.insert(order, tab:getText())
      end
    end
  end
  collect(consoleTabBar.preTabs)
  collect(consoleTabBar.tabs)
  collect(consoleTabBar.postTabs)
  g_settings.setNode('consoleTabOrder', order)
end

function applyTabOrder()
  if not consoleTabBar then return end
  local saved = g_settings.getNode('consoleTabOrder') or {}
  local userRank = {}
  for i, name in ipairs(saved) do
    if userRank[name] == nil then userRank[name] = i end
  end

  -- record each tab's current position so tabs that share the fallback rank
  -- (brand-new or private tabs) keep a stable order instead of shuffling
  local pos = {}
  local n = 0
  local function note(list)
    if not list then return end
    for _, tab in ipairs(list) do n = n + 1; pos[tab] = n end
  end
  note(consoleTabBar.preTabs)
  note(consoleTabBar.tabs)
  note(consoleTabBar.postTabs)

  local base = 10                    -- ordinary channels start after anchors 1..3
  local fallback = base + #saved + 1000
  local function rankOf(name)
    return anchorRank(name) or (userRank[name] and base + userRank[name]) or fallback
  end

  consoleTabBar:sortTabs(function(a, b)
    local ra, rb = rankOf(a:getText()), rankOf(b:getText())
    if ra ~= rb then return ra < rb end
    return (pos[a] or 0) < (pos[b] or 0)
  end)
end

-- Debounced reorder used when a tab appears after login (e.g. the Loot tab, or a
-- channel re-joining) so it snaps into its slot without fighting drags.
local function scheduleTabReorder()
  if not g_game.isOnline() then return end
  removeEvent(tabReorderEvent)
  tabReorderEvent = scheduleEvent(applyTabOrder, 50)
end
...
function onTabChange(tabBar, tab)
  ...
  -- switching to a channel jumps to its newest (bottom-most) message
...
function clear()
  -- remember the player's tab arrangement before the tabs are torn down
  saveTabOrder()
  removeEvent(tabReorderEvent)
  tabReorderEvent = nil
...
function addTab(name, focus)
  ...
  -- keep named tabs (Loot / NPCs) in their slot when they appear post-login
  if LOOT_TAB_NAMES[name] or name == 'NPCs' then
    scheduleTabReorder()
  end
  return tab
end
...
-- Splits text on {word} markers into ordered segments, each tagged with its
-- position (startPos/endPos) in the FINAL rendered string -- i.e. after braces
-- are stripped, which is what actually gets drawn and what label:getTextPos()
-- indexes into. Used both to build the coloring data below and, in addTabText,
-- to hit-test a click's text position back to the exact {word} it landed on.
function getHighlightSegments(text)
  local segments = {}
  local offset = 0

  local function addSegment(part, highlighted)
    segments[#segments + 1] = { text = part, highlighted = highlighted, startPos = offset, endPos = offset + #part }
    offset = offset + #part
  end

  for i, part in ipairs(text:split("{")) do
    if i == 1 then
      addSegment(part, false)
    else
      for j, part2 in ipairs(part:split("}")) do
        addSegment(part2, j == 1)
      end
    end
  end
...
function getNewHighlightedText(text, color, highlightColor)
  ...
  for _, segment in ipairs(getHighlightSegments(text)) do
    table.insert(tmpData, segment.text)
    table.insert(tmpData, segment.highlighted and highlightColor or color)
  end

  return tmpData
end

-- Maps a released left-click's screen position to whichever {word} segment (if
-- any) it landed on, and sends that word as chat instantly -- same as typing it
-- and pressing Enter (see sendMessage). No confirmation, no undo, so precision
-- matters: guards against firing off the tail end of a text-selection drag (see
-- onDragMove below) by requiring the release position match where the button
-- was originally pressed -- a real drag will have moved between press and release.
function onHighlightClick(label, mousePos, tab)
  local pressPos = label:getLastClickPosition()
  if pressPos and (mousePos.x ~= pressPos.x or mousePos.y ~= pressPos.y) then
    return false
  end

  local textPos = label:getTextPos(mousePos)
  for _, segment in ipairs(label.highlightSegments) do
    if segment.highlighted and textPos >= segment.startPos and textPos <= segment.endPos then
      sendMessage(segment.text, tab)
      return true
    end
  end
...
function addTabText(text, speaktype, tab, creatureName)
  ...
  -- Reset first -- labels get recycled (see MAX_LINES reuse above), so a stale
  -- highlightSegments list from a previous, unrelated line must not survive.
  label.highlightSegments = nil

  if speaktype.npcChat and (g_game.getCharacterName() ~= creatureName or g_game.getCharacterName() == 'Account Manager') then
    local segments = getHighlightSegments(text)
    local highlightData = {}
    for _, segment in ipairs(segments) do
      table.insert(highlightData, segment.text)
      table.insert(highlightData, segment.highlighted and "#1f9ffe" or speaktype.color)
    end
    if #highlightData > 2 then
      label:setColoredText(highlightData)
      label.highlightSegments = segments
    end
  end
  ...
  label.onMouseRelease = function(self, mousePos, mouseButton)
    if mouseButton == MouseLeftButton and label.highlightSegments and onHighlightClick(self, mousePos, tab) then
      return true
    end
...
function sendMessage(message, tab)
  ...
  -- when talking on server log (or the local-only Loot tab, which has no
  -- channelId of its own), the message goes to the default channel. Without this,
  -- Loot falls through to the private-message branch below and sends to a player
  -- literally named "Loot" -- silently eating spell words typed while that tab is
  -- focused, since they never reach the server as a spell cast.
  local name = tab:getText()
  if tab == serverTab or tab == getRuleViolationsTab() or LOOT_TAB_NAMES[name] then
    tab = defaultTab
...
function online()
  ...
  -- reopen last channels, in the player's saved left-to-right order so the tabs
  -- reappear deterministically instead of in a random hash order
  local lastChannelsOpen = g_settings.getNode('lastChannelsOpen')
  if lastChannelsOpen then
    local savedChannels = lastChannelsOpen[g_game.getCharacterName()]
    if savedChannels then
      local savedOrder = g_settings.getNode('consoleTabOrder') or {}
      local rank = {}
      ...
      for channelName, channelId in pairs(savedChannels) do
        table.insert(toOpen, { name = channelName, id = tonumber(channelId) })
      end
      table.sort(toOpen, function(a, b)
        local ra = rank[a.name] or 1000
        local rb = rank[b.name] or 1000
        if ra ~= rb then return ra < rb end
        return a.name < b.name
      end)
      for _, ch in ipairs(toOpen) do
        local channelId = ch.id
        if channelId and channelId ~= -1 and channelId < 100 then
  ...
  -- restore the saved tab order, then always land on the Default channel
