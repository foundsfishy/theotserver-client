...
function UIItem:onDragLeave(droppedWidget, mousePos)
  if self:isVirtual() then return false end
  -- Pop only if WE still hold the pushed cursor. onDestroy also pops when an item
  -- is destroyed mid-drag; without this guard the two double-pop 'target', which
  -- underflows the cursor stack and makes the crosshair stop appearing until relog.
...
function UIItem:onDestroy()
  -- if this item is destroyed mid-drag (e.g. its container closed), onDragLeave
  -- never fires -- pop the crosshair we pushed in onDragEnter so it can't stick
  if self.currentDragThing then
    g_mouse.popCursor('target')
    self.currentDragThing = nil
  end
