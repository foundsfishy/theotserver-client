...
function CharacterList.create(characters, account, otui)
  ...
  characterList.onChildFocusChange = function()
    -- scroll the list to keep the arrow-key-selected character on screen
    local focused = characterList:getFocusedChild()
    if focused then
      characterList:ensureChildVisible(focused)
    end
    removeEvent(autoReconnectEvent)
  ...
  if isPremium then
    local premiumText = tr('Premium Account')
    if account.premDays and account.premDays ~= 65535 then
      premiumText = premiumText .. '\n' .. ('(%d days left)'):format(account.premDays)
    end
    accountStatusLabel:setText(('%s%s'):format(premiumText, status))
  else
