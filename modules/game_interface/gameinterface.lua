...
function bindKeys()
  ...
  g_keyboard.bindKeyPress('Escape', function()
    ...
    -- cancel a pending "use with"/trade target first so Escape frees the crosshair
    if cancelUseWith() then return end
...
-- Cancel a pending "use with" / trade target: release the mouse grab and pop the
-- crosshair cursor. Returns true if something was cancelled. Without this, Escape
-- during targeting left the crosshair stuck on screen (Escape only stopped attack/
-- follow and never released the grab). selectedThing ~= nil means startUseWith/
-- startTradeWith pushed 'target' + grabbed, so the pop/ungrab stays balanced.
...
-- Custom wrap/unwrap channel (added 2026-07-22): this server's native
-- g_game.wrap() has nowhere to land - the engine's wrap system was stripped
-- from that fork's C++ entirely (declaration, event dispatch, and protocol
-- parsing all gone). Routes through the same extended-opcode channel this
-- server already uses for other custom features (e.g. real/uncapped exp),
-- just in the client->server direction. See data/lib/core/furniture_kits.lua
-- and data/creaturescripts/scripts/extendedopcode.lua on the server.
local WRAP_OPCODE = 66
function requestWrapItem(thing)
  local pos = thing:getPosition()
  local protocol = g_game.getProtocolGame()
  if not protocol then return end
  protocol:sendExtendedOpcode(WRAP_OPCODE, pos.x .. ',' .. pos.y .. ',' .. pos.z .. ',' .. thing:getId())
end

-- The Deco Seller's furniture kits were never given the Unwrapable/Wrapable
-- flag in this build's Tibia.dat (that binary predates the kits being sold
-- at all, and patching it by hand wasn't safe to do blindly) - so
-- isWrapable()/isUnwrapable() below never fire for them. Recognize them by
-- client id instead, mirroring the server's furniture_kits.lua.
local FURNITURE_KIT_CLIENT_IDS = {
  [2782]=true, [2788]=true, [2785]=true, [2787]=true, [2786]=true, [9061]=true,
  [2777]=true, [2775]=true, [2776]=true, [2778]=true, [2779]=true, [2780]=true, [2781]=true,
  [2789]=true, [2795]=true, [2790]=true, [2794]=true, [2791]=true, [6372]=true,
  [2798]=true, [2811]=true, [2797]=true, [2796]=true, [2806]=true, [2800]=true, [2805]=true,
  [6114]=true, [6115]=true, [2799]=true, [2804]=true, [2801]=true, [2803]=true,
  [2808]=true, [2807]=true, [10288]=true,
  -- bed colour kits (client ids 831-833, added 2026-07-23): now go through
  -- Unwrap too instead of "Use with" - the server auto-finds the bed under/
  -- near the player instead of requiring a manual target click. Bed removal
  -- kit (834/server 7907) REMOVED 2026-07-23 - confirmed buggy in testing.
  [831]=true, [832]=true, [833]=true,
}
local FURNITURE_REAL_CLIENT_IDS = {
  [2319]=true, [2350]=true, [2314]=true, [2348]=true, [2358]=true, [2374]=true,
  [2378]=true, [2382]=true, [2366]=true, [2418]=true, [2422]=true, [2465]=true, [2346]=true,
  [2441]=true, [2449]=true, [6367]=true, [2934]=true, [2979]=true, [2975]=true,
  [171]=true, [2997]=true, [2904]=true, [6111]=true, [6109]=true, [3484]=true,
  [2030]=true, [2445]=true, [2029]=true, [2963]=true, [2959]=true, [2982]=true,
  [2431]=true, [2483]=true, [9062]=true, [10286]=true,
  -- rotation variants (added 2026-07-23): Wrap now works no matter which
  -- way a rotatable piece (chair/table/statue/etc) is currently facing,
  -- not just its default south-facing id.
  [2351]=true, [2349]=true, [2347]=true, [2359]=true, [2360]=true, [2361]=true,
  [2377]=true, [2375]=true, [2376]=true, [2381]=true, [2379]=true, [2380]=true,
  [2385]=true, [2383]=true, [2384]=true, [2369]=true, [2367]=true, [2368]=true,
  [2421]=true, [2419]=true, [2420]=true, [2425]=true, [2423]=true, [2424]=true,
  [2434]=true, [2433]=true, [2432]=true, [2468]=true, [2467]=true, [2466]=true,
  [2444]=true, [2443]=true, [2442]=true, [2486]=true, [2484]=true, [2485]=true,
  [2452]=true, [2450]=true, [2451]=true, [6370]=true, [6369]=true, [6368]=true,
  [2998]=true, [2999]=true, [3000]=true, [6112]=true, [6110]=true,
  [3487]=true, [3485]=true, [3486]=true, [2048]=true, [2046]=true, [2047]=true,
  [2448]=true, [2447]=true, [2446]=true, [2045]=true, [2043]=true, [2044]=true,
  [2964]=true, [2962]=true, [2960]=true, [2961]=true, [10287]=true,
}
  ...
  if useThing then
    if useThing:isContainer() then
      if useThing:getParentContainer() then
        ...
        menu:addOption(tr('Open in new window'), function() modules.game_containers.openSmart(useThing) end)
      else
        menu:addOption(tr('Open'), function() modules.game_containers.openSmart(useThing) end, shortcut)
      end
    else
      local isFurnitureKitOrPiece = FURNITURE_KIT_CLIENT_IDS[useThing:getId()] or FURNITURE_REAL_CLIENT_IDS[useThing:getId()]
      if isFurnitureKitOrPiece then
        -- skip Use/Use-with for these - the Wrap/Unwrap option below covers it
    ...
    if useThing:isWrapable() or FURNITURE_REAL_CLIENT_IDS[useThing:getId()] then
      menu:addOption(tr('Wrap'), function() requestWrapItem(useThing) end)
    end
    if useThing:isUnwrapable() or FURNITURE_KIT_CLIENT_IDS[useThing:getId()] then
      menu:addOption(tr('Unwrap'), function() requestWrapItem(useThing) end)
    end
      ...
      if useThing then
        ...
        if useThing:isContainer() then
          if useThing:getParentContainer() then
            ...
            modules.game_containers.openSmart(useThing)
          end
          return true
        elseif useThing:isMultiUse() then
